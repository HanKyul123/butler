...
create table business_tb(
    ...
    business_pick_animal_type   int,            #대표동물
    business_min_charge       int,               #최소금액
    USER_NUM_FK            int,
...
create table review_tb(
    ...
    BUSINESS_PLACE_NUM_FK      int,
   constraint r_tb_BUSINESS_PLACE_NUM_FK foreign key(BUSINESS_PLACE_NUM_FK) references business_tb (BUSINESS_PLACE_NUM_PK),
    review_contents            varchar(5000),
    review_nickname            varchar(12),
    review_regdate             datetime   default now(),
    review_file_systemname      varchar(2000),
    review_file_orgname         varchar(2000)
);
...
create table business_charge_tb(
   BUSINESS_PLACE_NUM_FK      int,
   constraint bc_tb_BUSINESS_PLACE_NUM_FK foreign key(BUSINESS_PLACE_NUM_FK) references business_tb (BUSINESS_PLACE_NUM_PK),      
